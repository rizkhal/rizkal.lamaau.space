...
description: "Websocket 100 kali lebih baik dari pada setInterval"
tags: ["laravel", "vue"]
---

Laravel Reverb adalah _official package_ untuk membuat komunikasi _Real Time_ antara _Client_ dan _Server_.
...
Lanjut, buka file `app/Events/UserCreated.php` ubah channel yang digunakan.
...
Selanjutnya, kita daftarkan event yang telah kita buat untuk men-trigger broadcasting.
...
Selanjutnya buka file `Dashboard.vue`
...
Argunent dalam fungsi channel `(user-created)` adalah argument yang sama dalam method `broadcastOn` pada file `app/Events/UserCreated.php` dan argument dalam fungsi listen `(UserCreated)` adalah nama file event yang kita buat sebelumnya.
